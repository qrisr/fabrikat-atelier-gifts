...
export function LogoMark({ className }: { className?: string }) {
  return (
    <svg viewBox="0 0 40 40" className={cn("size-9", className)} aria-hidden="true">
      <rect x="1" y="1" width="38" height="38" rx="2" fill="none" stroke="currentColor" strokeWidth="1.2" />
      ...
      <path d="M20 12c-3-5-9-5-8-1 1 2 5 1 8 1zM20 12c3-5 9-5 8-1-1 2-5 1-8 1z" fill="none" stroke="var(--brand)" strokeWidth="1.2" />
    </svg>
  );
}
...
export function BrandMark({ compact = false }: { compact?: boolean }) {
  const { m } = useI18n();
  return (
    <div className={cn("flex min-w-0 items-center gap-3", compact && "justify-center")} aria-label={m.meta.appName}>
