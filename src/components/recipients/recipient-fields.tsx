...
export type DraftErrors = Partial<Record<"firstName" | "lastName" | "email" | "street" | "postalCode" | "city", string>>;
...
export function validateDraft(
  ...
  else if (options.takenEmails?.some((t) => t.toLowerCase() === draft.email.trim().toLowerCase())) errors.email = e.duplicate;
...
export function RecipientFields({
  ...
  const setAddress = (patch: Partial<SwissAddress>) => set({ address: { ...draft.address, ...patch } });
        ...
        <>
          <div className="grid gap-5 sm:grid-cols-2">
            <Field id={id("first")} label={f.firstName} error={errors.firstName}>
              <Input id={id("first")} autoComplete="given-name" value={draft.firstName} onChange={(e) => set({ firstName: e.target.value })} />
            </Field>
            <Field id={id("last")} label={f.lastName} error={errors.lastName}>
              <Input id={id("last")} autoComplete="family-name" value={draft.lastName} onChange={(e) => set({ lastName: e.target.value })} />
            </Field>
          </div>
          <Field id={id("email")} label={f.email} error={errors.email}>
            <Input id={id("email")} type="email" autoComplete="email" inputMode="email" value={draft.email} onChange={(e) => set({ email: e.target.value })} />
          </Field>
          {showCompany && (
            <Field id={id("company")} label={f.company}>
              <Input id={id("company")} autoComplete="organization" value={draft.company} onChange={(e) => set({ company: e.target.value })} />
            </Field>
          )}
        </>
      )}
      <Field id={id("street")} label={f.street} error={errors.street}>
        <Input id={id("street")} autoComplete="street-address" placeholder="Bahnhofstrasse 1" value={draft.address.street} onChange={(e) => setAddress({ street: e.target.value })} />
      </Field>
      <div className="grid grid-cols-[7rem_minmax(0,1fr)] gap-4 sm:grid-cols-[7rem_minmax(0,1fr)_10rem]">
        <Field id={id("plz")} label={f.postalCode} error={errors.postalCode}>
          <Input
            ...
            onChange={(e) => setAddress({ postalCode: e.target.value.replace(/\D/g, "").slice(0, 4) })}
          />
        </Field>
        <Field id={id("city")} label={f.city} error={errors.city}>
          <Input id={id("city")} autoComplete="address-level2" placeholder="Zürich" value={draft.address.city} onChange={(e) => setAddress({ city: e.target.value })} />
        </Field>
