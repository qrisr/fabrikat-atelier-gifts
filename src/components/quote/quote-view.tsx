...
export function QuoteView({
  ...
  const submit = async () => {
    ...
    if (result.ok) {
      ...
      toast.error(m.quote.submitError);
    }
  };
...
const REVIEW_STATUSES = ["submitted", "under_review", "approved", "changes_requested"] as const;

/** Fabrikat staff (or demo mode in the browser) moves a request through the review. */
function ReviewControls({ campaign }: { campaign: Campaign }) {
  const { m } = useI18n();
  const { isStaff, backend } = useAppState();
  const [busy, setBusy] = useState(false);
  if (!isStaff || campaign.status === "draft") return null;

  return (
    <section
      className="rounded-sm border border-dashed border-border p-5"
    ...
    >
      ...
      <p className="mt-1 text-sm text-muted-foreground">
        {backend === "supabase" ? m.review.intro : m.review.demoIntro}
      </p>
      <div className="mt-4 flex flex-wrap gap-2">
        {REVIEW_STATUSES.filter((status) => status !== campaign.status).map((status) => (
          <Button
            key={status}
            variant="outline"
            size="sm"
...
function ReviewStatus({
  ...
  const steps = [
    ...
    {
      key: "availability",
      done: campaign.status === "approved" || campaign.status === "changes_requested",
      active: campaign.status === "under_review",
    },
    {
      key: "personalization",
      done: campaign.status === "approved" || campaign.status === "changes_requested",
      ...
      <div>
        <div className="flex flex-wrap items-center gap-3">
          <p className="text-xs font-semibold uppercase tracking-[0.14em] text-brand">
            {m.quote.confirmationEyebrow}
          </p>
          <StatusBadge status={campaign.status} />
        </div>
        <h1 className="mt-4 font-display text-4xl leading-tight">{m.quote.confirmationTitle}</h1>
        <p className="mt-4 max-w-xl leading-7 text-muted-foreground">
          {m.quote.confirmationBody(contactEmail)}
        </p>
        <p className="mt-6 rounded-sm bg-secondary px-4 py-3 text-sm">
          {m.quote.statusPanel[campaign.status]}
        </p>
        {campaign.submittedAt && (
          <p className="mt-4 text-xs text-muted-foreground">
            {m.quote.submittedAt(dateTime(campaign.submittedAt))}
          </p>
        )}
...
function Summary({
  ...
  const engraved = template.items.filter(
    (item) => item.engravingSurcharge !== undefined && p.engravings[item.id]?.enabled,
  );
    ...
    <div className="space-y-8">
      ...
      <Block title={m.quote.presentation} action={edit("personalize")}>
        <div className="grid gap-6 sm:grid-cols-[14rem_minmax(0,1fr)]">
          ...
          <dl className="grid content-start gap-4 text-sm">
            ...
            <Row
              label={m.quote.message}
              value={p.cardMessage.trim() ? `«${p.cardMessage.trim()}»` : m.quote.noMessage}
            />
            <Row
              label={m.quote.logo}
              value={
            ...
            <Row
              ...
              value={
                engraved.length === 0
                  ...
                  : engraved
                      .map((item) => `${l(item.name)}: «${p.engravings[item.id]?.text || "–"}»`)
                      .join(" · ")
              }
            />
          </dl>
      ...
      <Block title={m.quote.recipients} action={edit("recipients")}>
        <dl className="grid gap-4 text-sm sm:grid-cols-2">
          <Row
            label={m.quote.recipients}
            value={`${number(recipients.length)} · ${m.quote.recipientsDetail(confirmed, recipients.length)}`}
          />
          <Row label={m.quote.delivery} value={date(campaign.deliveryDate)} />
        </dl>
        {editable && pending > 0 && (
          <p className="mt-4 text-sm text-muted-foreground">{m.quote.pendingAddresses(pending)}</p>
        )}
      </Block>
    </div>
  );
  ...
  return (
    ...
    >
      ...
      <p className="mt-2 text-xs text-muted-foreground">
        {m.quote.perRecipient(chf(cost.perRecipient))}
      </p>
