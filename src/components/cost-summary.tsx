...
export function CostSummary({
  ...
  return (
    <aside
      className="rounded-sm border border-border bg-card p-6 lg:sticky lg:top-8"
      aria-live="polite"
    >
      ...
      {!template ? (
        <p className="mt-6 border-t border-border pt-6 text-sm text-muted-foreground">
          {m.summary.noTemplate}
        </p>
      ) : (
        <>
          <dl className="mt-6 space-y-3 border-t border-border pt-6 text-sm">
            <Line
              label={`${m.summary.setPrice} · ${l(template.name)}`}
              value={chf(cost.setPrice)}
            />
            {detail === "full" && (
              <>
                {cost.wrapping > 0 && (
                  <Line label={m.summary.wrapping} value={`+ ${chf(cost.wrapping)}`} />
                )}
                {cost.sticker > 0 && (
                  <Line label={m.summary.sticker} value={`+ ${chf(cost.sticker)}`} />
                )}
            ...
            {detail === "full" && (
              <>
                ...
                {cost.logoSetup > 0 && (
                  <Line label={m.summary.logoSetup} value={chf(cost.logoSetup)} />
                )}
              </>
            )}
          </dl>
      ...
      <p className="mt-6 text-xs leading-5 text-muted-foreground">
        {actualRecipients > 0 ? m.summary.basedOnList : m.summary.basedOnEstimate}{" "}
        {m.summary.vatNote}
      </p>
